...
server.tool("addSubjects", "Add new subjects to an existing NATS stream", {
    ...
    try {
        const js = await nc.jetstreamManager({ domain: "local" });
...
server.tool("createStream", "Create a new NATS stream with specified configuration", {
...
server.tool("publish", "Publish a message to a NATS subject", {
...
server.tool("checkConsumerLag", "Check the lag status of a consumer in a stream", {
...
server.tool("listStreams", "List all streams on the NATS server", {}, async () => {
    try {
        const js = await nc.jetstreamManager({ domain: "local" });
...
server.tool("diagnoseStream", "Perform a comprehensive diagnosis of a NATS stream", {
    ...
    try {
        ...
        const report = [
            ...
            "",
            "👥 Connected Consumers:",
            ...consumerList.map(c => `• ${c.name} (${c.config.durable_name || "ephemeral"})`),
            "",
            "⚠️ Potential Issues:",
...
server.tool("diagnoseConsumer", "Perform a comprehensive diagnosis of a NATS consumer", {
